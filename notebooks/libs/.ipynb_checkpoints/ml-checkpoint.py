...
class AtmosphericalData:
    ...
    def convert_date(self):
        self.df["Fecha"] = self.df["Fecha"].astype(str) + "01"
        self.df["Fecha"] = pd.to_datetime(self.df["Fecha"], format="%Y%m%d")
    ...
    def data_preprocessing(self, n_steps: int = 12, train_vol: float = 0.9):
        ...
        sin_month = np.sin(2 * np.pi * self.df["Fecha"].dt.month / 12).values.reshape(-1, 1)
        cos_month = np.cos(2 * np.pi * self.df["Fecha"].dt.month / 12).values.reshape(-1, 1)
    ...
    def train_data(self, epochs=100, batch_size=360, validation_split=0.01, verbose=1):
        model = self._model
    ...
    def test_model(self):
        ...
        test_dates = self.df["Fecha"][self._n_steps + self._train_size:]
    ...
    def plot_tests(self):
        ...
        plt.plot(self.df["Fecha"], self.ds, label="Real", alpha=0.5)
    ...
    def predict_for_date(self, target_date: str):
        ...
        last_date = self.df["Fecha"].iloc[-1]
        ...
        sin_month = np.sin(2 * np.pi * self.df["Fecha"].dt.month / 12)
        cos_month = np.cos(2 * np.pi * self.df["Fecha"].dt.month / 12)
